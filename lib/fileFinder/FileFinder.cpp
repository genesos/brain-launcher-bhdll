...
int CFileFinder::Find(CFileFinder::CFindOpts &opts)
{
	...
	if (_pFileFinderProc != NULL)
		if(!_pFileFinderProc(this, FF_FOLDER))
		{
			_bStopSearch=1;
			return 0;
		}
...
*/
			...
			// Call callback procedure
			if (_pFileFinderProc != NULL)
				if(!_pFileFinderProc(this, bValidFile ? FF_FOUND : FF_DISCARDED))
				{
					_bStopSearch=1;
					return 0;
				}
...
BOOL CFileFinder::FindTextInFile(LPCTSTR szFile, LPCTSTR szText)
{
	...
	do
	{
		...
		if (_pFileFinderProc != NULL)
		{
			...
			{
				nLoopCount = 0;
				if(!_pFileFinderProc(this, FF_FINDTEXT))
				{
					_bStopSearch=1;
					return 0;
				}
...
void CFileFinder::SetCallback(CALLBACK_TYPE pFileFinderProc)
{
